 ...
 * License: GPLv3
 *
 * obbyircd note: imported from val's unrealircd-contrib channel-rename
 * (https://github.com/ValwareIRC/valware-unrealircd-mods/tree/main/channel-rename
 *  -- raw at unrealircd/unrealircd-contrib unreal6 files/channel-rename.c)
 * for in-tree shipment. The upstream Module-Manager preamble was dropped,
 * the module name lost its "third/" prefix, and MARK_AS_OFFICIAL_MODULE
 * was added on top of the existing MARK_AS_GLOBAL_MODULE. Everything
 * else (logic, RPC, PROTOCTL override, ModData layout) is unchanged.
 */
