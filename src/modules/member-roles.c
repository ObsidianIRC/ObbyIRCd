/*
  ...
  Copyright Ⓒ 2026 Valware & ObbyIRCd Team

  obbyircd port of upstream member-roles by Valware:
    https://github.com/ValwareIRC/valware-unrealircd-mods/blob/main/member-roles/member-roles.c

  Differences from upstream:
    - role registration sets the IRCv3 named-modes `name` field on
      CmodeInfo, so RPL_CHMODELIST advertises e.g. "+q owner" rather
      than letter-only.
    - rank check accepts negative values (UnrealIRCd's voice rank is -1).
    - adds CREGISTER <channel>: caller must have a services account,
      sets +P on the channel, persists registered_by/registered_at via
      the channeldb-owned "obsidianirc/channel-registration" ModData,
      and grants chanowner via `+e ~automode:q:~account:<owner>` so
      re-joins automatically promote the founder.
*/

#include "unrealircd.h"

/* Mirror of channeldb's per-channel registration ModData payload. */
...
MOD_INIT()
{
	...
	/* obbyircd: CREGISTER <#channel> -- register a channel and become
	 * its owner. See cmd_cregister at the bottom of this file. */
	CommandAdd(modinfo->handle, "CREGISTER", cmd_cregister, 1, CMD_USER);
...
MOD_LOAD()
{
	/* Locate the channel-registration ModData owned by channeldb.
	 * channeldb registers it in its MOD_INIT, which runs before us
	 * because channeldb is in core/. If channeldb isn't loaded
	 * (rare in obbyircd), CREGISTER will fail gracefully. */
	channel_registration_md = findmoddata_byname(CHANREG_MODDATA_NAME, MODDATATYPE_CHANNEL);

	/* Note: chanmode registration happens in member_roles_configrun
	 * (see "obbyircd: register the cmode now" comment), NOT here.
	 * MOD_LOAD fires immediately after MOD_INIT, before config blocks
	 * are processed, so member_roles is empty at this point. */
	return MOD_SUCCESS;
}

/* obbyircd: register a single role's CMODE_MEMBER right after parsing
 * it from configrun. Returns 1 on success, 0 on failure. */
static int register_role_cmode(struct MemberRole *role)
{
	CmodeInfo creq;
	if (!member_roles_modhandle)
		return 0;
	...
	role->cmode = CmodeAdd(member_roles_modhandle, creq, NULL);
	if (!role->cmode)
	{
 ...
 * CREGISTER <#channel>
 *
 * Make `channel` a registered channel and grant the caller chanowner.
 * Requirements:
 *   - caller is logged into a services account (we read client->user->account)
 *   - channel exists, caller is a member
 *   - channel is not already registered (no ChannelRegistration ModData)
 *
 * Effects:
 *   - sets +P on the channel (so channeldb persists the state)
 *   - stamps the ChannelRegistration ModData (registered_by + registered_at)
 *   - looks for the role with the highest rank and immediately gives it
 *     to the caller (this is the "owner" role in the default config)
 *   - adds an extban that re-grants the same mode to the same account
 *     on every join: +e ~automode:<mode>:~account:<account>
 * =================================================================== */

/* Find the highest-ranked role in the loaded config, NULL if none. */
static struct MemberRole *find_owner_role(void)
{
	struct MemberRole *role, *best = NULL;
	for (role = member_roles; role; role = role->next)
	{
...
CMD_FUNC(cmd_cregister)
{
	...
	Channel *channel;
	const char *account;
	struct MemberRole *owner_role;
	ChannelRegistration *reg;
	char extban_buf[256];
	char modeparam_buf[NICKLEN + 1];
	...
	{
		sendnumeric(client, ERR_NEEDMOREPARAMS, "CREGISTER");
		return;
	}
	...
	if (*chname != '#')
	{
		sendnotice(client, "*** CREGISTER: channel name must start with '#'");
		return;
	}

	channel = find_channel(chname);
	if (!channel)
	{
		sendnumeric(client, ERR_NOSUCHCHANNEL, chname);
		return;
	}
	...
	/* Must have a services account. UnrealIRCd stores it in
	 * client->user->account ("0" when unauthenticated). */
	if (!client->user || !*client->user->account || !strcmp(client->user->account, "0"))
	{
		sendnotice(client, "*** CREGISTER: you must be logged into a services account first (try /IDENTIFY).");
		return;
	}
	...
	{
		sendnotice(client, "*** CREGISTER: channel-registration storage is unavailable (channeldb module not loaded).");
		return;
	}
	...
	{
		sendnotice(client, "*** CREGISTER: %s is already registered to %s.", channel->name, reg->registered_by);
		return;
	}

	owner_role = find_owner_role();
	if (!owner_role)
	{
		sendnotice(client, "*** CREGISTER: no member roles configured (member-roles {} block missing).");
		return;
	}

	/* Set +P so channeldb starts persisting the channel. */
	...
	reg->registered_at = TStime();
	moddata_channel(channel, channel_registration_md).ptr = reg;

	/* Grant the owner role to the caller right now. */
	{
		const char *modeargs[3];
		char modestr[3];
		modestr[0] = '+';
		modestr[1] = owner_role->mode;
		modestr[2] = '\0';
		strlcpy(modeparam_buf, client->name, sizeof(modeparam_buf));
		modeargs[0] = modestr;
		modeargs[1] = modeparam_buf;
		modeargs[2] = NULL;
		do_mode(channel, &me, NULL, 3, modeargs, 0, 0);
	}

	/* Auto-promote on rejoin via ~automode:<mode>:~account:<account>
	 * placed on the +e (except) list. */
	{
		const char *modeargs[3];
		snprintf(extban_buf, sizeof(extban_buf), "~automode:%c:~account:%s", owner_role->mode, account);
		modeargs[0] = "+e";
		modeargs[1] = extban_buf;
		modeargs[2] = NULL;
		do_mode(channel, &me, NULL, 3, modeargs, 0, 0);
	}

	sendnotice(client, "*** %s is now registered to your account (%s). You are the channel owner.", channel->name, account);
	unreal_log(ULOG_INFO, "channel-registration", "CHANNEL_REGISTERED", client,
	           "$client.details registered $channel as account $account",
