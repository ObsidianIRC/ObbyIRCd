/*
 * Stores user metadata (recognising users by their services account) and
 * channel metadata across IRCd restarts.  Pairs with the metadata module
 * (draft/metadata-2): when a metadata key/value is set on a logged-in
 * client or on a +P channel, this module persists it to disk; on next
 * start (or on next ACCOUNT_LOGIN for users) we replay the stored keys
 * back via a METADATA command, so clients seeing those entities reconcile
 * the metadata the same way they would for a live SET.
 *
 * Imported from k4be's third/metadata-db (pirc-pl/unrealircd-modules) and
 * bundled as a first-party module.  Original (C) 2019 Syzop, Gottem and the
 * UnrealIRCd team; metadata-db forked from channeldb.c by k4be.
 *
 * License: GPLv2
 */
...
void store_metadata(char *account, struct metadata *metadata, time_t last_seen){
	...
	curr = safe_alloc(sizeof(struct metadata_storage));
