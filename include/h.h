...
extern SSL_CTX *tls_ctx_for_listener(ConfigItem_listen *listener);
extern SSL_CTX *tls_ctx_for_outgoing_link(ConfigItem_link *link);
extern TLSOptions *tls_options_for_listener(ConfigItem_listen *listener);
extern TLSOptions *tls_options_for_outgoing_link(ConfigItem_link *link);
extern void test_tlsblock(ConfigFile *conf, ConfigEntry *cep, int *totalerrors);
extern void set_server_linking_tlsoptions_ce(ConfigEntry *ce);
...
extern const char *display_path(const char *path, const char *reldir);
...
extern int is_trusted_cert(SSL_CTX *ctx);
...
extern EVENT(update_known_user_cache_timer);
